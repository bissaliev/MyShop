...
class Recommender:
    ...
    def get_product_key(self, id):
        """
        Метод получает id объекта Product и формирует ключ.
        """
        return f"product:{id}:purchased_with"

    def products_bought(self, products):
        """
        Метод получает список объектов Product, которые были куплены вместе.
        """
    ...
    def suggest_products_for(self, products, max_results=6):
        """
        Метод извлечения товаров, которые были куп­лены вместе
        для списка заданных товаров.
        """
        product_ids = [p.id for p in products]
        if len(products) == 1:
            # только 1 товар
